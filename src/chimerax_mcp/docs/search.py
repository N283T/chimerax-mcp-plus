...
class DocSearch:
    ...
    def build_index(self) -> dict[str, int]:
        ...
        if not self._docs_path.exists():
            raise FileNotFoundError(f"Documentation path does not exist: {self._docs_path}")
        ...
        for html_file in html_files:
            ...
            try:
                ...
                logger.warning("Failed to parse %s: %s, skipping", relative, e)
                skipped += 1
                continue
    ...
    def lookup(self, command_name: str) -> list[dict[str, Any]]:
    ...
    def ensure_index(self) -> None:
        ...
        if not self.is_indexed():
            ...
            try:
                ...
                raise RuntimeError(f"Failed to build documentation index: {e}") from e
